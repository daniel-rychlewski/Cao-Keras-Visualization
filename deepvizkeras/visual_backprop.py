from deepvizkeras.saliency import SaliencyMask
import numpy as np
...
class VisualBackprop(SaliencyMask):
    """
    unused because expansion for models for hyperspectral datasets is not at all trivial
    A SaliencyMask class that computes saliency masks with VisualBackprop (https://arxiv.org/abs/1611.05418).
    """

    def __init__(self, model, output_index=0, n_bands=100):
        ...
        self.layer_number = output_index # used in base class
        ...
        self.compute_gradients = K.function(inputs=input_tensors, outputs=gradients) # used in base class
        self.n_bands = n_bands # used in base class
    ...
    def get_mask(self, input_image):
        ...
        visual_bpr = None
        x_value = x_value.squeeze(axis=4)
    ...
    def _deconv(self, feature_map):
        ...
        # temp=deconv_func([feature_map, 0])[0]
        return deconv_func([feature_map, 0])[0]

    # def get_grad(self, input_image):
    #     # Execute the function to compute the gradient
    #     x_value = np.expand_dims(input_image, axis=0)
    #     x_value.reshape((1,5,5,100))
    #     if self.layer_number is not None:
    #         if self.layer_number == 0:
    #             x_value = x_value.squeeze(axis=4)
    #         else:# self.layer_number == 1 or self.layer_number == 2:
    #             x_value = x_value.reshape((1, 5, 5, self.n_bands))
