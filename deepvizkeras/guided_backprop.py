...
class GuidedBackprop(SaliencyMask):
    ...
    def __init__(self, model, output_index=0, custom_loss=None, n_bands=100):
        """Constructs a GuidedBackprop SaliencyMask."""

        super().__init__(model=model, layer_number=None, output_index=output_index, n_bands=n_bands)
        ...
        tmph5path ='./gb_keras.h5'
        guidedbackproppath = './guided_backprop_ckpt'
        guidedbackpropmetapath = './guided_backprop_ckpt.meta'

        model.save(tmph5path)
        with tf.Graph().as_default(): 
            with tf.Session().as_default(): 
                K.set_learning_phase(0)
                load_model(tmph5path, custom_objects={"custom_loss":custom_loss})
                session = K.get_session()
                tf.train.export_meta_graph()
                
                saver = tf.train.Saver()
                saver.save(session, guidedbackproppath)
        ...
        with self.guided_graph.as_default():
            ...
            with self.guided_graph.gradient_override_map({'Relu': 'GuidedRelu'}):
                saver = tf.train.import_meta_graph(guidedbackpropmetapath)
                saver.restore(self.guided_sess, guidedbackproppath)
